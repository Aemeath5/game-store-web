...
const metrics = [
  ...
  { label: '今日订单', value: '46', change: '+8.4%', icon: ShoppingCart, tone: 'green' },
  { label: '今日成交额', value: '¥ 38,420', change: '+18.6%', icon: CircleDollarSign, tone: 'gold' },
  { label: '活跃用户', value: '1,217', change: '+6.2%', icon: Users, tone: 'purple' },
]
...
const orders = [
  ...
  { id: 'GM202608070029', user: '枫丹来客', item: '最初的大魔术', amount: '¥ 2,680', status: '人工复核', tone: 'info' },
...
const pending = [
  ...
  { title: '提现申请待复核', count: 6, hint: '合计 ¥ 12,860', icon: CircleDollarSign },
]
        ...
        <table class="admin-table">
          ...
          <tbody>
            <tr v-for="order in orders" :key="order.id">
              <td><strong>{{ order.id }}</strong></td><td>{{ order.user }}</td><td>{{ order.item }}</td><td>{{ order.amount }}</td>
